...
(() => {
  ...
  /* ---------- Formulaire de devis : envoi AJAX ----------
     Amélioration progressive : sans JS, le formulaire poste
     normalement vers contact.php (redirection vers merci.html). */
  ...
  if (contactForm) {
    ...
    const submitLabel  = contactForm.querySelector('[data-form-submit-label]');
    ...
    contactForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (response.ok && data.ok) {
          contactForm.reset();
          // Réinitialise le champ d'estimation dynamique
          if (typeof updateEstimateField === 'function') updateEstimateField();
          setStatus('ok', data.message || 'Merci, votre demande a bien été envoyée.');
          statusEl?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        } else {
          setStatus('error', data.message || 'Une erreur est survenue. Merci de réessayer.');
        }
      } catch (error) {
        setStatus('error', 'Connexion impossible. Merci de réessayer ou de nous appeler au 04 90 80 05 05.');
      } finally {
